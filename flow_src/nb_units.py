...
def data_to_plot(data_dict):
    ...
    for index_ax, (ax, tech_node) in enumerate(zip(axes, tech_nodes)):
        baseline_areas[tech_node] = {}

        # First loop to find the baseline area for each category and bitwidth in the current technology node
        for design, tech_data in data_dict.items():
            if 'baseline' in design and tech_node in tech_data:
        ...
        for design, tech_data in data_dict.items():
            if tech_node in tech_data:
                ...
                if baseline_area is None or current_area is None:
                    continue  # Skip if no baseline area is found for the category or current area is None

                value = baseline_area / current_area
                print(tech_node, design, baseline_area, current_area, value)
                marker_shape = category_to_marker.get(category, 'x')
                adder_size = get_adder_size_from_name(design)
                latency = safe_float(tech_data[tech_node].get("latency", None))

                # Use color based on adder size
                color = "black" if adder_size is None else cmap(norm(adder_size))

                ax.scatter(latency, value, label=f"{category} {adder_size} bit",
    ...
    fig.supxlabel('Latency (Clock cycles)')
    fig.supylabel(rf"Number of Div. Units for a baseline budget")

    plt.tight_layout()
    plt.savefig(f"nb_units_across_technodes_comparison.pdf", bbox_inches='tight')
    plt.close(fig)
